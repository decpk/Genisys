import {
  MOCKSERVER_READ_TOOL_NAMES,
  MOCKSERVER_TOOL_DEFINITIONS,
  MOCKSERVER_TOOL_REGISTRY,
} from '@/ai/tools/mock-server'
...
/**
 * Drive the Mock Server AI assistant through the shared frontend agentic
 * loop. This executes Mock-Server tools (projects, servers, endpoints,
 * variants, start/stop, ports, request logs, navigation, etc.) in-browser
 * against the Zustand store — so the assistant can perform everything a user
 * can do in the UI.
 *
 * The transport (`createDailyPlanTransport`) is a generic frontend
 * completion transport, despite the name — it is reused here.
 */
export async function runMockServerAI(
  systemPrompt: string,
  ...
  const readNames = new Set(MOCKSERVER_READ_TOOL_NAMES)
  ...
  if (!USE_NEW_AGENTIC_LOOP) {
    callbacks.onError(
      'The Mock Server AI assistant requires the frontend agentic loop to be enabled.',
    )
    return
  }
  ...
  return agenticLoop({
    ...
    userMessage,
    tools: filteredTools,
    toolRegistry: MOCKSERVER_TOOL_REGISTRY,
