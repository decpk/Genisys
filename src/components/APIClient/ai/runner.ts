...
/**
 * Drive the API Client AI assistant through the shared frontend agentic
 * loop. This executes API-Client tools (collections, folders, requests,
 * environments, sending requests, navigation, etc.) in-browser against the
 * Zustand store — so the assistant can perform everything a user can do in
 * the UI.
 *
 * The transport (`createDailyPlanTransport`) is a generic frontend
 * completion transport, despite the name — it is reused here.
 */
export async function runApiClientAI(
  systemPrompt: string,
  ...
  const readNames = new Set(API_CLIENT_READ_TOOL_NAMES)
  ...
  if (!USE_NEW_AGENTIC_LOOP) {
    callbacks.onError(
      'The API Client AI assistant requires the frontend agentic loop to be enabled.',
    )
    return
  }
  ...
  return agenticLoop({
    ...
    userMessage,
    tools: filteredTools,
    toolRegistry: API_CLIENT_TOOL_REGISTRY,
