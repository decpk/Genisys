...
function callCompletion(
  ...
  return new Promise((resolve, reject) => {
    ...
    // Reuse the DailyPlan AI completion endpoint — it's a generic OpenAI proxy
    // Event filtering is by streamId, so no cross-talk
...
export async function runClipboardAI(
  systemPrompt: string,
  ...
  const isToolAllowed = createIsToolAllowedForMode(mode, CLIPBOARD_READ_TOOL_NAMES)

  if (USE_NEW_AGENTIC_LOOP) {
    // Clipboard reuses the DailyPlan completion endpoint (generic OpenAI proxy).
    return agenticLoop({
      systemPrompt,
      conversationHistory: conversationHistory as never,
      userMessage,
      tools: filteredTools,
      toolRegistry: CLIPBOARD_TOOL_REGISTRY,
  ...
  for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
    ...
    for (const tc of result.toolCalls) {
      ...
      const toolModule = CLIPBOARD_TOOL_REGISTRY[tc.name]
      if (!toolModule) {
