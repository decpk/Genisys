...
import { ENTITY_TOKEN_PROMPT_RULE } from "@/ai/entity-links";
import { buildToolBudgetGuidance } from "@/prompts";
...
export function buildClipboardSystemPrompt(): string {
  ...
  const budgetGuidance =
    ...
    buildToolBudgetGuidance(
      "the clipboard manager (items, labels, statistics)",
    );

  return (
    BASE_PROMPT +
    "\n\n" +
    ENTITY_TOKEN_PROMPT_RULE +
    contextBlock +
    budgetGuidance
  );
}
