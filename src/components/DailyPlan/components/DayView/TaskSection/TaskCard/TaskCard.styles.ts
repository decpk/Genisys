...
export const taskCardStyles = {
  // Borderless, transparent list row — rows sit flush in the section well
  // with only a subtle hover tint (see `itemTileElevationStyles`). Priority
  // is expressed by a single calm dot (see `PriorityDot`).
  card: `group/task relative isolate overflow-hidden ${itemTileElevationStyles.base}`,
  ...
  cardRow:
    "flex items-center gap-2 relative z-[1] transition-[padding] duration-200 ease-out group-hover/task:pr-7",
  cardTitle:
  ...
  overdueIndicator:
    "shrink-0 inline-flex items-center gap-1 text-[10px] font-semibold text-red-400",
  ...
  menuButton:
    "absolute top-1.5 right-1.5 flex items-center justify-center size-6 rounded-md bg-background/70 ring-1 ring-inset ring-border/40 backdrop-blur-sm opacity-0 group-hover/task:opacity-100 focus-within:opacity-100 hover:bg-background/90 transition-all duration-150 z-10",
} as const;

export const STATUS_PILL_STYLES: Record<string, string> = {
  in_progress: taskCardStyles.statusPillInProgress,
  completed: taskCardStyles.statusPillCompleted,
}
