...
export function GridView(props: ViewModeComponentProps): React.JSX.Element {
  const {
    items,
    currentPath,
    sort,
...
const GridRow = memo(function GridRow(props: GridRowProps): React.JSX.Element {
  const {
    item,
    index,
    isActive,
  ...
  return (
    ...
    >
      <button
        ...
        style={buttonStyle}
        className={`group relative flex flex-col items-center gap-1.5 p-3 w-[120px] h-[100px] rounded-md text-foreground font-normal transition-colors ${cursorClass} ring-1 ring-border/0 hover:ring-border/60 hover:bg-secondary/40 data-[active=true]:bg-primary/10 data-[active=true]:ring-primary/30 data-[active=true]:text-primary`}
      >
        <span className="shrink-0 relative">
          {getFileIcon(name, item.isFolder, 32)}
          {isIcloud && (
            <Cloud
              size={12}
        ...
        <span className="text-xs w-full text-center overflow-hidden text-ellipsis whitespace-nowrap">
          {name}
        </span>
        {ext && <ExtPill ext={ext} />}
