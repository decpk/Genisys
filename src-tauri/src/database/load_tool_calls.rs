...
/// Load all tool calls for a conversation, ordered by started_at then sort_order.
pub fn load_tool_calls_db(db: &Database, conversation_id: &str) -> Vec<ToolCallRecord> {
    let conn = db.reader();
    let mut stmt = match conn.prepare(
        ...
        Ok(s) => s,
        Err(e) => {
            eprintln!("[db] load_tool_calls prepare: {e}");
            return Vec::new();
        }
    };
