...
#[tauri::command]
pub async fn cmd_dailyplan_ai_completion(
    app: tauri::AppHandle,
    stream_id: String,
    messages: Vec<Value>,
    tools: Vec<Value>,
    ...
    if let Err(e) = result {
        let _ = app.emit("dailyplan-ai-error", serde_json::json!({
            "streamId": stream_id,
            "error": e
        }));
    }
...
async fn run_completion(
                ...
                while let Some(pos) = sse_buffer.find('\n') {
                    ...
                    if let Ok(parsed) = serde_json::from_str::<Value>(data) {
                        ...
                        if let Some(token) = delta["content"].as_str() {
                            ...
                            let _ = app.emit("dailyplan-ai-chunk", serde_json::json!({
                        ...
                        if let Some(rtok) = reasoning_token {
                            if !rtok.is_empty() {
                                let _ = app.emit("dailyplan-ai-reasoning-chunk", serde_json::json!({
    ...
    let _ = app.emit("dailyplan-ai-done", serde_json::json!({
        "streamId": stream_id,
